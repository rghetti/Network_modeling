...
printSiena <- function(siena.fit) {
  ...
  temp.rates <- data.frame(
    dependent = "rate",
    effect = attributes(siena.fit$f)$condEffects$effectName,
    theta = round(siena.fit$rate, 3),
    s.e. = round(siena.fit$vrate, 3),
    p.value = 0,
    sig. = 0, t.conv = 0
  )
  ...
  siena.fit$pvalues <- 2 *
    (1 - pnorm(abs(siena.fit$theta / sqrt(diag(siena.fit$covtheta)))))
  stars <- ifelse(
    siena.fit$pvalues <= 0.10,
    ifelse(
      siena.fit$pvalues <= 0.05,
      ifelse(
        siena.fit$pvalues <= 0.01,
        ifelse(
          siena.fit$pvalues <= 0.001, "***", "**"
        ), "*"
      ), "."
    ), ""
  )
  ...
  temp.effects <- data.frame(
    dependent = effects[, c("name")],
    effect = effects[, c("effectName")],
    theta = round(siena.fit$theta, 3),
    s.e. = round(sqrt(diag(siena.fit$covtheta)), 3),
    p.value = round(siena.fit$pvalues, 3),
    sig. = stars,
  ...
  siena.results.temp[
    siena.results.temp$dependent == "rate", c("p.value", "sig.", "t.conv")
  ] <- ""
...
printSienaCoev <- function(siena.fit, nperiods) {
  ...
  res <- data.frame(
    effect = c(siena.fit$f$basicEffects[[1]][, 2],
               siena.fit$f$basicEffects[[2]][, 2]),
    theta = round(siena.fit$theta, 3),
    s.e. = round(sqrt(diag(siena.fit$covtheta)), 3),
    p.value = round(pvalues, 3),
    sig. = stars,
    t.conv = round(siena.fit$tconv, 3)
  )
